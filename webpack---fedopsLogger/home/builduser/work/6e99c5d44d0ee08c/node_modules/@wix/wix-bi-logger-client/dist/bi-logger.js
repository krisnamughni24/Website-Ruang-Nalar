...
var BiLogger = (function () {
  ...
  function BiLogger(options, context) {
    ...
    this._eventTransformer =
      options.eventTransformer ||
      function (event) {
        return event;
      };
    this._payloadTransformer =
      options.payloadTransformer ||
      function (payload) {
        return payload;
      };
    this._consentPolicyGetter =
      options.consentPolicyGetter ||
      function () {
        return null;
      };
    ...
    [
      ...
      },
      {
        ...
        value: function log(
          ...
          assert.defined(
            eventOrKey,
            "Event object or event key must be provided."
          );

          var _extractEventAndConte = this._extractEventAndContext(
              eventOrKey,
              eventOrContextOrUndefined,
              contextOrUndefined
            ),
          ...
          if (
            this._isMuted() ||
            shouldMuteByCategory(policy, fullContext.category)
          ) {
          ...
          if (fullContext.useBatch) {
            ...
            var transformAndQueue = function transformAndQueue(_event) {
              var transformedEvent = _this._eventTransformer(
                _event,
                fullContext
              );
            ...
            if (this._globalBatchQueue) {
              return this._getDefaults(this._defaults).then(function (
                ...
                var fullEvent = _extends(
                  {},
                  defaults,
                  _this._getDynamicNonEssentialDefaults(policy),
                  _this._getStaticNonEssentialDefaults(policy),
                  event,
                  _this._getPolicyFields(policy, fullContext.category)
                );
          ...
          return this._getDefaults(this._defaults).then(function (defaults) {
            ...
            var validatorsResult =
              _this._validators.length === 0
                ? true
                : _this._validators.some(function (validator) {
                    return (
            ...
            if (!validatorsResult) {
              throw new Error(
                "No validator accepted the event. Source: " +
                  fullEvent.src +
                  " Evid: " +
                  (fullEvent.evid || fullEvent.evtId)
              );
            }
            ...
            transformedEvent = _this._payloadTransformer(
              transformedEvent,
              fullContext
            );
      ...
      },
      {
        ...
        value: function _send(payload) {
          ...
          return Promise.all(
            this._publishers.map(function (publisher) {
      ...
      },
      {
        ...
        value: function _extractEventAndContext(
          eventOrKey,
          eventOrContextOrUndefined,
          contextOrUndefined
        ) {
          ...
          var onFlush = function onFlush(batch) {
            ...
            if (!_this3._globalBatchQueue) {
              batch.g = Object.assign(
                _this3._getStaticDefaults(_this3._defaults),
                _this3._getStaticNonEssentialDefaults(policy)
              );
            }
          ...
          var promises = mapValues(defaults, function (value, key) {
            ...
            if (value && typeof value.then === "function") {
              return promise
                .timedPromise(value, {
                  message:
                    "Cannot get default value '" + key + " for BI Event'",
