...
export const isScript = ({ initiatorType, name }) =>
  initiatorType === "script" ||
  (initiatorType === "link" && /\.js$/.test(name));
/** @type {PerformanceEntryFilter} */
export const isAjax = ({ initiatorType }) =>
  initiatorType === "fetch" || initiatorType === "xmlhttprequest";
/** @type {PerformanceEntryFilter} */
export const isImage = ({ initiatorType, name }) =>
  initiatorType === "img" ||
  initiatorType === "image" ||
  (initiatorType === "css" && /\.(?:jpe?g|png|webp|gif)/i.test(name));
/** @type {PerformanceEntryFilter} */
export const isFont = ({ initiatorType, name }) =>
    ...
    observer = observe(
      ...
      (entries) => {
        entries = entries.filter(
          ({ name }) => !ignores.some((ignore) => name.includes(ignore))
        );
        if (entries.length) {
          cbs.forEach((cb) =>
