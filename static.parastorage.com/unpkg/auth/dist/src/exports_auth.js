...
fireauth.exportlib.exportPrototypeMethods(fireauth.Auth.prototype, {
  ...
  signInWithEmailLink: {
    ...
    args: [
      fireauth.args.string("email"),
      fireauth.args.string("emailLink", true),
    ],
  },
  ...
  useEmulator: {
    name: "useEmulator",
    args: [fireauth.args.string("url"), fireauth.args.object("options", true)],
  },
...
fireauth.exportlib.exportPrototypeMethods(fireauth.grecaptcha.prototype, {
  execute: {
    name: "execute",
  },
  render: {
...
fireauth.exportlib.exportPrototypeProperties(fireauth.AuthSettings.prototype, {
...
fireauth.exportlib.exportFunction(
  ...
  [fireauth.args.string("email"), fireauth.args.string("password")]
);
...
fireauth.exportlib.exportFunction(
  ...
  [fireauth.args.or(fireauth.args.string(), fireauth.args.object(), "token")]
);
fireauth.exportlib.exportFunction(
  fireauth.EmailAuthProvider,
  "credentialWithLink",
  fireauth.EmailAuthProvider.credentialWithLink,
  [fireauth.args.string("email"), fireauth.args.string("emailLink")]
);
...
fireauth.exportlib.exportPrototypeMethods(fireauth.OAuthCredential.prototype, {
  toPlainObject: {
    name: "toJSON",
    // This shouldn't take an argument but a blank string is being passed
    // on JSON.stringify and causing this to fail with an argument error.
...
fireauth.exportlib.exportPrototypeMethods(fireauth.MultiFactorError.prototype, {
  toJSON: {
    name: "toJSON",
    // This shouldn't take an argument but a blank string is being passed
    // on JSON.stringify and causing this to fail with an argument error.
...
fireauth.exportlib.exportPrototypeMethods(fireauth.MultiFactorUser.prototype, {
  getSession: {
    name: "getSession",
    args: [],
  },
...
(function () {
  if (
    ...
    throw new Error(
      "Cannot find the firebase namespace; be sure to include " +
    ...
    const authInteropComponent = {
      name: "auth-internal",
      instanceFactory: function (container) {
        var auth = container["getProvider"](fireauth.exportlib.AUTH_TYPE)[
          "getImmediate"
        ]();
        return {
          getUid: goog.bind(auth.getUid, auth),
          getToken: goog.bind(auth.getIdTokenInternal, auth),
          addAuthTokenListener: goog.bind(
            auth.addAuthTokenListenerInternal,
            auth
          ),
          removeAuthTokenListener: goog.bind(
            auth.removeAuthTokenListenerInternal,
            auth
          ),
        };
      },
