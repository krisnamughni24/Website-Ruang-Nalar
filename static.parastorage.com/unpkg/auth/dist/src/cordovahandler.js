 ...
 */
fireauth.CordovaHandler = function (
  authDomain,
  apiKey,
  appName,
  clientVersion,
   ...
   */
  this.getAndDeletePartialEventManager_ = new fireauth.storage.AuthEventManager(
    this.storageKey_
  );
  /**
  ...
  this.initialTimeout_ =
    initialTimeout || fireauth.CordovaHandler.INITIAL_TIMEOUT_MS_;
  /** @private {number} The return to app after redirect timeout. */
  this.redirectTimeout_ =
    redirectTimeout || fireauth.CordovaHandler.REDIRECT_TIMEOUT_MS_;
  /**
...
fireauth.CordovaHandler.prototype.initializeAndWait = function () {
  ...
  this.isReady_ = fireauth.util.checkIfCordova().then(
    function () {
      ...
      var appIdentifier = fireauth.util.getObjectRef(
 ...
 * @override
 */
fireauth.CordovaHandler.prototype.startPopupTimeout = function (
  popupWin,
  onError,
  timeoutDuration
) {
...
fireauth.CordovaHandler.prototype.processRedirect = function (
  ...
  if (this.pendingRedirect_) {
    return goog.Promise.reject(
      new fireauth.AuthError(fireauth.authenum.Error.REDIRECT_OPERATION_PENDING)
    );
  }
  var self = this;
  ...
  this.pendingRedirect_ = goog.Promise.resolve()
    .then(function () {
      // Validate provider.
...
fireauth.CordovaHandler.prototype.processRedirectInternal_ = function (
  ...
  var appDisplayName = fireauth.util.getObjectRef(
  ...
  return this.initializeAndWait()
    .then(function () {
      ...
      return self.savePartialEventManager_.setAuthEvent(
        self.storageKey_,
        event
      );
    })
      ...
      isAvailable(function (result) {
        if (result) {
          ...
          openUrl = /** @type {!function(string, string, string=)} */ (
            fireauth.util.getObjectRef("cordova.InAppBrowser.open", goog.global)
          );
          if (typeof openUrl !== "function") {
            throw new fireauth.AuthError(
              fireauth.authenum.Error.INVALID_CORDOVA_CONFIGURATION
...
fireauth.CordovaHandler.prototype.getPartialStoredEvent_ = function () {
  ...
  return this.getAndDeletePartialEventManager_
    .getAuthEvent()
 ...
 * @private
 */
fireauth.CordovaHandler.prototype.extractAuthEventFromUrl_ = function (
  partialEvent,
  url
) {
  ...
  var universalLinkCb = function (eventData) {
    ...
    self.getPartialStoredEvent_().then(function (event) {
      ...
      if (event && eventData && eventData["url"]) {
        ...
        authEvent =
          self.extractAuthEventFromUrl_(event, eventData["url"]) || noEvent;
      }
      // Dispatch Auth event.
 ...
 * @override
 */
fireauth.CordovaHandler.prototype.removeAuthEventListener = function (
  listener
) {
