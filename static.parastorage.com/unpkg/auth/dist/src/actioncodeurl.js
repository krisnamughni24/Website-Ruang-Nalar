...
fireauth.ActionCodeURL = function (actionLink) {
  ...
  var apiKey =
    uri.getParameterValue(fireauth.ActionCodeURL.QueryField.API_KEY) || null;
  var code =
    uri.getParameterValue(fireauth.ActionCodeURL.QueryField.CODE) || null;
  var mode =
    uri.getParameterValue(fireauth.ActionCodeURL.QueryField.MODE) || null;
  ...
  if (!apiKey || !code || !operation) {
    throw new fireauth.AuthError(
      ...
      fireauth.ActionCodeURL.QueryField.API_KEY +
        ", " +
        fireauth.ActionCodeURL.QueryField.CODE +
        "and " +
  ...
  fireauth.object.setReadonlyProperties(this, {
    ...
    code: code,
    continueUrl:
      uri.getParameterValue(fireauth.ActionCodeURL.QueryField.CONTINUE_URL) ||
      null,
    languageCode:
      uri.getParameterValue(fireauth.ActionCodeURL.QueryField.LANGUAGE_CODE) ||
      null,
    tenantId:
      uri.getParameterValue(fireauth.ActionCodeURL.QueryField.TENANT_ID) ||
      null,
  });
};
